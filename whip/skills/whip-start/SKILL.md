...
- If an agent failed and you want to preserve context, notes, or session history, use `whip task retry <id>`
- Use `whip task unassign <id>` when you need to kill or reset a live or stuck task before retrying from scratch
...
2. Agent works: the prompt instructs it to avoid committing and to report via `whip task status <id> review` when done
