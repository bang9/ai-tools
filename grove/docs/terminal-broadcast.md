...
7. Mirror a PTY: the mirror and the source pane share **one** daemon session, and the mirror runtime becomes its **resize owner**. There is no second client and nothing clamps the session — the daemon applies exactly what the owner asks. But the source pane's xterm keeps its own container grid, so while the mirror is live the source pane reads back an applied size it did not choose and that will never converge on its fitted grid. Its applied-size reassertion (`docs/terminal-resize.md`) must treat that as the non-convergence case — adopt it and STOP — with **no re-fit oscillation or flicker loop** in the source pane. On mirror end the PTY restores to `originalCols/originalRows` as usual.
