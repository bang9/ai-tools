...
function MissionItem({ mission }: Props) {
  ...
  const deleting = useMissionStore((s) => !!s.deletingMissions[mission.id]);
  ...
  const addingMissionProjects = useMissionStore((s) => s.addingMissionProjects);

  const pendingProjectIds = useMemo(
    () =>
      Object.keys(addingMissionProjects)
        .filter((key) => key.startsWith(`${mission.id}:`) && addingMissionProjects[key])
  ...
  const isMissionSelected = selectedItem?.missionId === mission.id && !selectedItem?.projectId;
    ...
    <SidebarContextMenu path={mission.missionDir} noteKey={noteKey} noteLabel={mission.name}>
      <div className={cn("px-1.5")}>
        ...
        >
          ...
          >
            {mission.name}
            <NoteEmoji noteKey={noteKey} label={mission.name} />
          </span>
          ...
          >
            ...
            {deleting ? (
              ...
              <IconButton onClick={handleDelete} title="Delete mission">
