...
export function filterUrlSuggestions(
  list: string[],
  query: string,
  limit = 8,
): string[] {
  ...
  const matched = q
    ? list.filter((url) => url.toLowerCase().includes(q))
    : list;
 ...
 */
export function findUrlCompletion(
  list: string[],
  typed: string,
): UrlCompletion | null {
  ...
  for (const url of list) {
    ...
    if (
      stripped.length > strippedTyped.length &&
      stripped.toLowerCase().startsWith(needle)
    ) {
