...
fn shell_command(shell: &str, interactive: bool, command: &str) -> Command {
    ...
    // Own process group, so a timeout can reap the WHOLE probe — not just the shell
    // pid. An interactive shell sources the user's rc, which may spawn children (a
    // plugin, a backgrounded job); killing only `child.id()` would orphan those.
    ...
    loop {
            ...
            .map_err(|e| format!("Failed to poll shell: {e}"))?
        {
            ...
            None if start.elapsed() >= LOGIN_SHELL_COMMAND_TIMEOUT => {
                // Kill the whole process group (the shell is its group leader, so its
                // pid == the pgid), reaping any children the rc spawned; then reap the
                // shell itself. SAFETY: `child.id()` is our own just-spawned child.
